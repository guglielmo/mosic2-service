...
function setIdFascicoliRegistri() {
    ...
    $query = "SELECT r.*, f.id as fascicolo_id FROM `msc_registri` as r
              LEFT JOIN msc_fascicoli as f ON r.numero_fascicolo = f.codice_repertorio";
    $res = mysqli_query($db, $query);

    //aggiorno tabella
    if (mysqli_num_rows($res) >= 1) {
        while ($row = mysqli_fetch_array($res)) {
            $query = 'UPDATE `msc_registri` SET `id_fascicoli`= "' . $row['fascicolo_id'] . '"  WHERE id = "' . $row['id'] . '"';
            echo $query . "<br>";
            mysqli_query($db, $query);
        }
    }
...
function aggiornaUfficialeRiunione($id, $stato) {
    global $db;

    $query = 'UPDATE `msc_precipe` SET `ufficiale_riunione`= "' . $stato . '"  WHERE id = "' . $id . '"';
    $res2 = mysqli_query($db, $query);
}
function aggiornaUfficialeRiunioneCipe($id, $stato) {
    global $db;

    $query = 'UPDATE `msc_cipe` SET `ufficiale_riunione`= "' . $stato . '"  WHERE id = "' . $id . '"';
    $res2 = mysqli_query($db, $query);
}
